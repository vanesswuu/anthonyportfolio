      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-24">
           <div>
             ...
             <h2 className="font-heading text-6xl md:text-8xl font-black tracking-tighter text-white uppercase leading-none">Global <br/><span className="text-accent italic font-accent lowercase font-normal">Press</span></h2>
           </div>
           <p className="text-neutral-400 max-w-xs mt-8 md:mt-0 md:text-right font-light italic font-accent text-xl leading-relaxed">
             "A voice shaping the global real estate narrative across the most respected publications."
           </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-white/10 border border-white/10">
          {news.map((item, i) => (
            <a 
              ...
              className="press-item group bg-black p-12 hover:bg-white/5 transition-all duration-500 relative overflow-hidden"
            >
              <div className="flex justify-between items-start mb-12">
                <span className="text-[10px] tracking-[0.3em] uppercase font-bold text-accent">
                  {item.source}
                </span>
                <span className="text-[10px] tracking-widest text-neutral-600 font-mono font-bold">
                  {item.date}
                </span>
              </div>
              
              <h3 
                className="font-heading text-2xl md:text-3xl font-black leading-tight transition-colors duration-500 text-white uppercase"
              >
                {item.title}
              </h3>
