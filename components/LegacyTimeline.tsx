...
export default function LegacyTimeline() {
  ...
  return (
    <section ref={containerRef} id="legacy" className="py-64 px-6 bg-black text-white relative overflow-hidden border-t border-white/5">
      <div className="max-w-7xl mx-auto text-center mb-32">
        <span className="text-accent text-[10px] tracking-[0.6em] uppercase font-bold mb-6 block">The Journey</span>
        <h2 className="font-heading text-6xl md:text-9xl font-black tracking-tighter text-white uppercase leading-none">THE <span className="text-accent italic font-accent font-normal text-7xl md:text-[120px] lowercase">Legacy</span></h2>
      </div>
        ...
        <div className="space-y-48 relative">
          {timeline.map((item, i) => (
            <div key={i} className={`timeline-item flex items-center justify-between w-full ${i % 2 === 0 ? 'flex-row-reverse left' : 'right'}`}>
              <div className="w-[45%]">
                <div className={`p-8 border border-white/5 bg-white/5 backdrop-blur-sm relative group hover:border-accent transition-colors duration-500 rounded-3xl ${i % 2 === 0 ? 'text-left' : 'text-right'}`}>
                  <span className="font-heading text-4xl md:text-6xl font-black text-accent mb-4 block">{item.year}</span>
                  <h3 className="font-heading text-xl md:text-2xl font-black text-white uppercase tracking-tight mb-4">{item.title}</h3>
                  <p className="text-neutral-400 text-sm leading-relaxed font-normal">{item.desc}</p>
